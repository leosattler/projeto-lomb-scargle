...
#------------------------------------
plt.subplot(2,1,1)
plt.title('Yearly averaged signal (uniformly sampled)')
...
freqs, aft_shifted = psd(f, sr, 2**5)
plt.plot(freqs, aft_shifted,'k-')
freqs_real = freqs[int(len(aft_shifted)/2)+1:]
best_frequency = freqs_real[np.argmax(aft_shifted[int(len(aft_shifted)/2)+1:])]
plt.vlines(sr*best_frequency, 0, 1.1*np.max(aft_shifted), label = 'freq. = '+str(round(sr*best_frequency,4))+','+'\n' + 'period = '+str(round(1/(sr*best_frequency),4)), linestyle='dashed')
plt.ylim(0,1000)
plt.legend(loc=0)
plt.xscale('log')
#------------------------------------
